...
def get_latest_features_for_ticker(ticker, sma_windows, support_window, feature_cols):
...
if run_analysis:
    ...
    # Price charts with SMAs and RSI
    with tab3:
    ...
    with tab4:
        try:
            ...
            if X.empty or y.empty:
                st.warning("Not enough historical data to train the ML model with current settings.")
            else:
                ...
                # Predict for latest data of selected tickers
                rows = []
                for t in selected_tickers:
                    row = get_latest_features_for_ticker(t, sma_tuple, support_window, feature_cols)
                    if row is None:
                        continue
            ...
            st.error("scikit-learn not available. Please install it to train and run ML model.")
